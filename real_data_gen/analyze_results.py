...
f = open(f'./real_data_gen/results.txt', 'a')
...
f.write(f'tn: {tn}\n')
f.write(f'fn: {fn}\n')
f.write(f'Percent Out of Vocab: {out_of_vocab}/{num_rows} = {round((out_of_vocab)/num_rows, 4)}\n')
...
f.write(f'f1: {2*tp}/{2*tp+fp+fn} = {round((2*tp)/(2*tp+fp+fn), 4)}\n\n')
f.close()